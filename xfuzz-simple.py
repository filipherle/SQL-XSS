...
def main():    
    if (len(sys.argv) != 2 or XSSCHECKVAL not in sys.argv[1]):
        exit("Usage: python xfuzz-simple.py <FULL URL REPLACING PARAM WITH " + XSSCHECKVAL + ">\nExample: python xfuzz-simple.py http://site.com/?param=" + XSSCHECKVAL + "\n")
    global URL
    URL = sys.argv[1]
    ...
    if(XSSCHECKVAL.lower() in init_resp.lower()):
        ...
        print "[+] Response contains parameter value! In code " + str(NUM_REFLECTIONS) + " time(s)."
